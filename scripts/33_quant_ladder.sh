...
printf '%-8s %8s %10s\n' quant size_GB ppl
# baseline F16
f16_ppl=$("$LC/build/bin/llama-perplexity" -m "$F16" -f "$EVAL" -ngl 999 -c 512 2>/dev/null | grep -oE 'Final estimate: PPL = [0-9.]+' | grep -oE '[0-9.]+$')
...
for q in "${QUANTS[@]}"; do
  ...
  ppl=$("$LC/build/bin/llama-perplexity" -m "$out" -f "$EVAL" -ngl 999 -c 512 2>/dev/null | grep -oE 'Final estimate: PPL = [0-9.]+' | grep -oE '[0-9.]+$')
